...
/** Backend supports link-only portfolio items (no /portfolio/with-file route). */
export async function createPortfolioWithFile(userId, item, file) {
  void file
  const payload = portfolioPayloadFromUi(item)
  if (!isValidPortfolioUrl(payload.link)) {
    throw new Error('Portfolio file upload requires a valid project URL on the backend')
  }
  return createPortfolioItem(userId, item)
}
...
/** Backend has no portfolio file endpoints — link metadata only. */
 ...
 */
/** Skill catalog — GET /v1/mentors/catalog (replaces removed listAllSkill route) */
export async function fetchAllSkills() {
  if (!isApiEnabled()) return []
  const { skills } = await fetchMentorCatalog()
  return skills
}
...
/** Province list — GET /v1/mentors/catalog (replaces removed provinces/listAll route) */
export async function fetchProvinces() {
  if (!isApiEnabled()) return []
  const { provinces } = await fetchMentorCatalog()
  return provinces
}
