...
import { createCommunityPost, fetchCommunityById } from '@/services/communities/communityService'
...
const CreatePostModal = ({ open, onClose }) => {
  ...
  const [pickedCommunity, setPickedCommunity] = useState(null)

  const selectedCommunity = pickedCommunity
  ...
  const handleContinue = async () => {
    if (!selected) return
    const community = await fetchCommunityById(selected)
    if (community) setPickedCommunity(community)
    setStep(2)
  }
  ...
  const handleShare = async () => {
    const postTitle =
      ...
      hashtag.trim()
    if (!selected || !postTitle) return
    setSubmitting(true)
    setSubmitError(null)
    try {
      if (isApiEnabled()) {
        await createCommunityPost({
          communityTypeId: selected,
          title: postTitle,
          description: description.trim() || hashtag.trim() || location.trim() || null,
        })
      }
      onClose()
    } catch (err) {
      setSubmitError(err?.message || 'Could not publish post')
    } finally {
      setSubmitting(false)
    }
  }
          ...
          <div className="flex flex-col flex-1 min-h-0 create-post-step-panel-in bg-white">
            <div className="flex-1 min-h-0 overflow-y-auto overscroll-contain px-4 py-3 space-y-3">
              ...
              <input
                ...
                placeholder={t('communityPost.title') || 'Post title'}
                className="w-full px-4 py-2.5 rounded-xl border border-slate-200/90 bg-white text-sm font-semibold text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-primary-200"
              />
            ...
            <div className="flex-shrink-0 px-4 sm:px-5 py-3.5 border-t border-slate-100 bg-white/90 flex items-center justify-between gap-3">
              ...
              >
                {submitting ? t('common.loading') : t('communityPost.sharePost')}
              </Button>
              {submitError && <p className="text-xs text-red-600 w-full">{submitError}</p>}
            </div>
          </div>
        )}
      </div>
